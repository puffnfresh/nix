    ...
    Descriptor)
{
    ...
    if (useSandbox)
        return makeFreeBSDChrootDerivationBuilder(store, std::move(miscMethods), std::move(params));

    return makeFreeBSDDerivationBuilder(store, std::move(miscMethods), std::move(params));
#else
    if (useSandbox)
